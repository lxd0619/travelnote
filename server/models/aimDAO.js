...
var aimDAO = {
    ...
    aimPersonalRow: function (strategy,callback) {
        DAO('select * from personalrow where personalrow.cityName like ? and prStatus>=0 order by strategyId desc limit 6', [strategy.cityName], function (err, results) {
            if (err) {
                callback(err, null)
            } else {
                callback(null, results)
    ...
    aimSceneryStrategy: function (strategy,callback) {
        DAO('select * from scenerystrategy where scenerystrategy.cityName like ? and ssStatus>=0 order by strategyId desc', [strategy.cityName], function (err, results) {
            if (err) {
                callback(err, null)
            } else {
                callback(null, results)
    ...
    aimFoodStrategy: function (strategy,callback) {
        DAO('select * from foodstrategy where foodstrategy.cityName like ? and fsStatus>=0 order by strategyId desc limit 6', [strategy.cityName], function (err, results) {
            if (err) {
                callback(err, null)
            } else {
                callback(null, results)
