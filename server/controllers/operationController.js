...
var operationController={
    ...
    Report: function (req, res) {
        ...
        switch (reportInfo.strategyType) {
            case 'scenerystrategy': sql = '  update scenerystrategy set ssStatus=1 where strategyId=? and (ssStatus=0 or ssStatus=1)'; break;
            case 'foodstrategy': sql = '  update foodstrategy set fsStatus=1 where strategyId=? and (fsStatus=0 or fsStatus=1)'; break;
            case 'personalrow': sql = ' update personalrow set prStatus=1 where strategyId=? and (prStatus=0 or prStatus=1)'; break;
        }
    ...
    normalStrategy:function(req,res){
        ...
        switch (normalStrategyInfo.strategyType) {
            case 'scenerystrategy': sql = ' select * from scenerystrategy where (ssStatus=0 or ssStatus=1) order by ssTime desc '; break;
            case 'foodstrategy': sql = ' select * from foodstrategy where (fsStatus=0 or fsStatus=1) order by fsTime desc '; break;
            case 'personalrow': sql = '  select  * from  personalrow  where (prStatus=0 or prStatus=1) order by prTime desc '; break;
        }
    ...
    hotStrategy: function (req, res) {
        ...
        switch (hotStrategyInfo.strategyType) {
            case 'scenerystrategy': sql = ' select * from scenerystrategy where ssStatus=0 order by ssLikeNum desc,ssCollectionNum desc'; break;
            case 'foodstrategy': sql = ' select * from foodstrategy order by fslikenum desc,fscollectionnum desc'; break;
            case 'personalrow': sql = '  select distinct * from users join personalrow on users.userId=personalrow.userId where (prStatus=0 or prStatus=1) order by personalrow.prLikeNum desc limit 3'; break;
        }
    ...
    strategyDetail: function (req, res) {
        ...
        switch (strategyInfo.strategyType) {
            case 'scenerystrategy': sql = 'select * from scenerystrategy left join comments on scenerystrategy.strategyId = comments.strategyId  and comments.strategyType="scenerystrategy" left join replys on comments.commentId = replys.commentId where scenerystrategy.strategyId = ? '; break;
            case 'foodstrategy': sql = ' select * from foodstrategy left join comments on foodstrategy.strategyId = comments.strategyId  and comments.strategyType="foodstrategy" left join replys on comments.commentId = replys.commentId where foodstrategy.strategyId = ?'; break;
            case 'personalrow': sql = ' select * from personalrow left join comments on personalrow.strategyId = comments.strategyId  and comments.strategyType="personalrow" left join replys on comments.commentId = replys.commentId where personalrow.strategyId = ?'; break;
        }
