  ...
  <div>
    <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
      <el-tab-pane label="粉丝" name="first">
        <div>
          <ul>
            <li v-for="li in list" :key="li.index" @click="go(li.userId)">
              <img
      ...
      <el-tab-pane label="关注" name="second">
        <div>
          <ul>
            <li v-for="group in attentionslist" :key="group.index"  >
              <img
              ...
              <el-button type="warning" v-if="show" @click="addFriends(group.userId)" style="float:right; margin-top:15px; margin-right:15px;">取消关注</el-button>
            </li>
          </ul>
        </div>
      </el-tab-pane>
