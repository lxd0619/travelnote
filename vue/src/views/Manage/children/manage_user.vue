...
export default {
  ...
  created() {
    this.$axios.post("http://localhost:3000/manage/usersSelect").then(res => {
      ...
      for (var i = 0; i < res.data.data.length; i++) {
        ...
        } else if (res.data.data[i].userStatus > 0) {
  ...
  methods: {
    ...
    push() {
      ...
      if (this.radio == 0) {
        var message =
          "@" + this.userId + ":您收到一条系统消息<br>您的账号已恢复正常";
      } else if (this.radio == 10) {
        var message =
          "@" +
          this.userId +
          ":您收到一条系统警告消息<br>由于您的攻略多次被举报，经审核，予以您警告";
      } else {
        var message =
          "@" +
          this.userId +
          "：您收到一条系统禁言消息<br>更具《中华人民共和国网络安全法》和《互联网信息服务管理办法》规定，经核实，您已违规，现予以您禁言处理";
      }
      this.$axios
        .post("http://localhost:3000/manage/sendMessage", {
